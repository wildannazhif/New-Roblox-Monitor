...
export default function App() {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          if (filesProcessed === totalFiles) {
            setOrders(prev => {
              const combined = [...prev, ...allNewOrders];
              const uniqueMap = new Map<string, EldoradoOrder>();
              combined.forEach(o => uniqueMap.set(o.orderId, o));
  ...
  const summary = useMemo((): SalesSummary => {
    ...
    const daysInInterval = startDate && endDate 
      ? eachDayOfInterval({ start: parseISO(startDate), end: parseISO(endDate) }).length
      : 1;
    ...
    return {
      ...
      averageDailyTransactions: filteredByDate.length / Math.max(1, daysInInterval),
    };
